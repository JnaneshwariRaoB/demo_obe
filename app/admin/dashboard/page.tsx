    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-[#2C2233] to-[#3F2D47] p-8 font-serif text-white font-bold">
      <div className="w-full max-w-5xl bg-[#4B3B5C] p-8 rounded-3xl shadow-2xl border-4 border-[#A786DF]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Left Side - Existing Departments */}
          <div className="bg-[#3F3147] p-6 rounded-xl shadow-md border border-[#9C88B8]">
            <h3 className="text-2xl text-center mb-4 text-white font-bold">Departments</h3>
            {departments.length > 0 ? (
              <ul className="space-y-4 max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-[#9C88B8] scrollbar-track-transparent">
                {departments.map((dept, index) => (
                  <li
                    key={index}
                    className="p-4 bg-[#5A476D] rounded-lg border border-[#8C7BAF] hover:bg-[#6A5985] transition text-white font-bold"
                  >
                    {editIndex === index ? (
              ...
              <p className="text-center text-white font-bold">No departments added</p>
            )}
          </div>

          {/* Right Side - Add New Department */}
          <div className="bg-[#3F3147] p-6 rounded-xl shadow-md border border-[#9C88B8]">
            <h3 className="text-2xl text-center mb-4 text-white font-bold">Add Department</h3>
            <input
              type="text"
              placeholder="Department Code"
              value={newDepartment.code}
              onChange={(e) => setNewDepartment({ ...newDepartment, code: e.target.value.toUpperCase() })}
              className="w-full p-3 mb-3 rounded-xl border border-[#9C88B8] bg-[#5A476D] text-white font-bold placeholder-white focus:ring-2 focus:ring-[#A786DF] focus:outline-none shadow-sm"
            />
            <input
              type="text"
              ...
              className="w-full p-3 rounded-xl border border-[#9C88B8] bg-[#5A476D] text-white font-bold placeholder-white focus:ring-2 focus:ring-[#A786DF] focus:outline-none shadow-sm"
            />
            <button
              onClick={addDepartment}
              className="w-full mt-4 py-2 bg-gradient-to-r from-[#A786DF] to-[#6A4C93] text-white font-bold rounded-xl hover:from-[#5D4A75] hover:to-[#9370DB] transition shadow-md"
            >
              Save
            </button>
          </div>
        ...
        <button
          ...
          className="w-full mt-10 py-3 bg-[#6A4C93] text-white font-bold rounded-xl hover:bg-[#5D4A75] transition duration-300 shadow-md"
        >
          Next
        </button>
      </div>
